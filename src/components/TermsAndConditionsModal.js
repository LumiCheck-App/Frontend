...
export default function TermsAndContitionsModal({ modalVisible, setModalVisible }) {
  ...
  return (
    <Modal animationType="slide" transparent={true} visible={modalVisible} className="relative">
      {/* Overlay */}
      ...
      <View className="h-screen justify-center items-center w-screen px-5">
        <View className="relative max-h-96 bg-off-white pt-20 pb-10 px-6 flex-col justify-center items-center w-full border-x border-y border-light-gray rounded-lg gap-8">
          <Text className="text-2xl font-quickbold text-yellow">Termos e Condições</Text>
          ...
          <TouchableOpacity className="ml-2 absolute top-4 right-4" onPress={CloseModal}>
